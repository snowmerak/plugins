...
fn main() -> std::io::Result<()> {
	...
	if count > 1 {
		println!("Starting Rust Ringbuffer Demo as {} (Benchmark: count={}, size={})...", role, count, size);
	} else {
	...
	if count > 1 {
		...
		if role == "Host" {
			...
			println!("BENCHMARK_RESULT: {} rounds, total time: {:?}, {:.2} ops/sec, avg latency: {:.2} us", count, elapsed, ops, latency);
		} else {
			for _ in 0..count {
				reader.read_exact(&mut read_buf)?;
				writer.write_all(&payload)?;
